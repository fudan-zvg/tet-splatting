...
class RandomCameraDataModuleConfig:
    ...
    train_max_step: float = 10000  # using to identity each view in validation
    train_val_check: float = 200  # using to identity each view in validation
    n_gpus: int = 1
    camera_sampling_stone: List[int] = field(default_factory=lambda: [])
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        # for conditional camera setttings
...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        if self.split == "val":
            ...
            self.view_step = 0
            view_iter = self.cfg.train_max_step // self.cfg.train_val_check
            azimuth_deg = torch.linspace(0, 360.0, int(view_iter) + 1)
            self.n_views = int(view_iter) + 1
        else:
        ...
        identity_c2w = repeat(identity_c2w, "1 w h -> b w h", b=c2w.shape[0])
        _, rays_d_tmp = get_rays(directions, identity_c2w, keepdim=True)
        self.depth_scale = torch.abs(rays_d_tmp[..., 2:])
    ...
    def __getitem__(self, index):
        return {
            ...
            "c2w": self.c2w[index],
            "rays_depth_scale": self.depth_scale[index],
...
class RandomCameraMultiDataset(RandomCameraDataset):
    def __getitem__(self, index):
        back_index = (index + self.n_views // 2) % self.n_views
        ...
        back = {
            "index": back_index,
            "rays_o": self.rays_o[back_index],
            "rays_d": self.rays_d[back_index],
            "c2w": self.c2w[back_index],
            "rays_depth_scale": self.depth_scale[back_index],
            "mvp_mtx": self.mvp_mtx[back_index],
            "camera_positions": self.camera_positions[back_index],
            "light_positions": self.light_positions[back_index],
            "elevation": self.elevation[back_index],
            "azimuth": self.azimuth[back_index],
            "camera_distances": self.camera_distances[back_index],
        }

        return {"front": front, "back": back}


class RandomCameraStepDataset(RandomCameraDataset):
    def __len__(self):
        return self.n_views * self.cfg.n_gpus

    def __getitem__(self, index):
        index = self.view_step
        view_step = self.view_step
        self.view_step = (self.view_step + 1) % self.n_views
...
class RandomMultiViewCameraStepDataset(RandomCameraDataset):
    """using to see backview"""
    ...
    def __getitem__(self, index):
        index = self.view_step
        multi_index = (self.view_step + self.n_views // 2) % self.n_views
        ...
        back = {
            "index": multi_index,
            "rays_o": self.rays_o[multi_index],
            "rays_d": self.rays_d[multi_index],
            "c2w": self.c2w[multi_index],
            "rays_depth_scale": self.depth_scale[multi_index],
            "mvp_mtx": self.mvp_mtx[multi_index],
            "camera_positions": self.camera_positions[multi_index],
            "light_positions": self.light_positions[multi_index],
            "elevation": self.elevation[multi_index],
            "azimuth": self.azimuth[multi_index],
            "camera_distances": self.camera_distances[multi_index],
            "view_step": multi_index,
        }
